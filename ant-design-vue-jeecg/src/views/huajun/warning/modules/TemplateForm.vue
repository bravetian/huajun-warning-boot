        ...
        <a-row>
          <a-col :span="24">
            <a-form-item label="模板名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['tname']" placeholder="请输入模板名称"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/warning/template/add",
          edit: "/warning/template/edit",
          queryById: "/warning/template/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'tname','wlever','wtype','status'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'tname','wlever','wtype','status'))
      },
    }
  }
</script>